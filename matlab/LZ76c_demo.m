...
% e.g. (cut & paste)
%{
maxn = 100000; maxq = 5; fs = 200; oudec = 1; LZ76c_demo
%}
...
for q = 1:maxq                         % for each quantisation
	...
	c(:,q) = LZ76c_x(s);               % calculate "running" LZ complexity (i.e., for all sequence lengths to maximum)
...
semilogx(n,cnorm);
ylim([0 1.2]);
line([1 maxn],[1 1],'color','k');
yline(1,'color','k');
title('normalised LZ76-complexity');
